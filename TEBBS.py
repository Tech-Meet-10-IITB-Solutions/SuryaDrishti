...
def TEBBS_calculate(start_time, end_time, plot_key = 0, sys_win = 0, savitzky_golay = 0, extend_50 = 0):
    ...
    jbest = jbest[0]
    if (extend_50 == 0):
        #print(plot_temp.shape[0], ibest[0], jbest)
        #print(plot_em.shape[0])
        Tmax = numpy.amax(plot_temp[5*int(plot_temp.shape[0]/36):5*int(plot_temp.shape[0]/6),ibest,jbest])
        EMmax = numpy.amax(plot_em[5*int(plot_em.shape[0]/36):5*int(plot_em.shape[0]/6),ibest,jbest])
        Tmax_time = ftiming_ext[5*int(plot_temp.shape[0]/36) + numpy.argmax(plot_temp[5*int(plot_temp.shape[0]/36):5*int(plot_temp.shape[0]/6),ibest,jbest])]
        EMmax_time = ftiming_ext[5*int(plot_temp.shape[0]/36) + numpy.argmax(plot_em[5*int(plot_em.shape[0]/36):5*int(plot_em.shape[0]/6),ibest,jbest])]
    else:
        Tmax = numpy.amax(plot_temp[int(plot_temp.shape[0]/9):2*int(plot_temp.shape[0]/3),ibest,jbest])
        EMmax = numpy.amax(plot_em[int(plot_em.shape[0]/9):2*int(plot_em.shape[0]/3),ibest,jbest])
        Tmax_time = ftiming_ext[plot_temp.shape[0]/9 + numpy.argmax(plot_temp[int(plot_temp.shape[0]/9):2*int(plot_temp.shape[0]/3),ibest,jbest])]
        EMmax_time = ftiming_ext[plot_temp.shape[0]/9 + numpy.argmax(plot_em[int(plot_em.shape[0]/9):2*int(plot_em.shape[0]/3),ibest,jbest])]
    ...
    if (extend_50 == 0):
        ...
        AFluxMax = numpy.amax(numpy.copy(fluxes_out[5*int(flxlength/36):5*int(flxlength/6),0]))
        BFluxMax = numpy.amax(numpy.copy(fluxes_out[5*int(flxlength/36):5*int(flxlength/6),1]))
        AFluxTime = ftiming_ext[5*int(flxlength/36) + numpy.argmax(numpy.copy(fluxes_out[5*int(flxlength/36):5*int(flxlength/6),0]))]
        BFluxTime = ftiming_ext[5*int(flxlength/36) + numpy.argmax(numpy.copy(fluxes_out[5*int(flxlength/36):5*int(flxlength/6),1]))]
    else:
        if (len(fluxes_out.shape) > 2):
            print ("ALERT: array reformation. Check the results.")
            fluxes_out = numpy.copy(fluxes_out[:,0,:])
        flxlength = len(numpy.copy(fluxes_out[:,0]))
        AFluxMax = numpy.amax(numpy.copy(fluxes_out[int(flxlength/9):2*int(flxlength/3),0]))
        BFluxMax = numpy.amax(numpy.copy(fluxes_out[int(flxlength/9):2*int(flxlength/3),1]))
        AFluxTime = ftiming_ext[flxlength/9 + numpy.argmax(numpy.copy(fluxes_out[int(flxlength/9):2*int(flxlength/3),0]))]
        BFluxTime = ftiming_ext[flxlength/9 + numpy.argmax(numpy.copy(fluxes_out[int(flxlength/9):2*int(flxlength/3),1]))]
